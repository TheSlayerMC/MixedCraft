...
public class ChunkGeneratorFlylight implements IChunkProvider {
	...
	private ArrayList<WorldGenMinable> ores, lowerOres;
	private ArrayList<WorldGenFlowers> flowers;
	private ArrayList<WorldGenFlylightReed> sugarCane;

	public ChunkGeneratorFlylight(World par1World, long par2) {
		this.worldObj = par1World;
		this.type = par1World.getWorldInfo().getTerrainType();
		this.rand = new Random(par2);

		lowerOres = new ArrayList(5);
		lowerOres.add(add(BlockHelper.diamond, 6));
		lowerOres.add(add(BlockHelper.emerald, 3));
		lowerOres.add(add(BlockHelper.gold, 6));
		lowerOres.add(add(BlockHelper.lapis, 4));
		lowerOres.add(add(BlockHelper.redstone, 6));
		
		ores = new ArrayList(2);
		ores.add(add(BlockHelper.iron, 10));
		ores.add(add(BlockHelper.coal, 20));
		
		flowers = new ArrayList(4);
		flowers.add(new WorldGenFlowers(BlockHelper.flower1));
		flowers.add(new WorldGenFlowers(BlockHelper.flower2));
		flowers.add(new WorldGenFlowers(BlockHelper.FlylightTallGrass));
		flowers.add(new WorldGenFlowers(BlockHelper.flylightFern));
		
		sugarCane = new ArrayList(1);
		sugarCane.add(new WorldGenFlylightReed());
	...
	private WorldGenMinable add(Block toSpawn, int amount){
		return new WorldGenMinable(toSpawn, amount, BlockHelper.flylightStone);
	}
						...
						for(int i3 = 0; i3 < 4; ++i3) {
							...
							for(int k3 = 0; k3 < 4; ++k3) {
								if((d15 += d16) > 0.0D) 
									...
									b[j3 += short1] = BlockHelper.flylightGrass;
								else 
									b[j3 += short1] = null;
	...
	public final void genBiomeTerrain(World p_150560_1_, Random p_150560_2_, Block[] p_150560_3_, byte[] p_150560_4_, int p_150560_5_, int p_150560_6_, double p_150560_7_, BiomeGenBase b)
	{
		...
		for (int l1 = 255; l1 >= 0; --l1)
		{
			...
			else
			{
				...
				{
					if (block2 == BlockHelper.flylightStone)
					{
						if (k == -1)
						{
							if (l <= 0)
							{
								block = null;
								b0 = 0;
								block1 = BlockHelper.flylightStone;
							}
							else if (l1 >= 59 && l1 <= 64)
							{
								block = b.topBlock;
							...
							if (l1 < 63 && (block == null || block.getMaterial() == Material.air))
							{
								...
								{
									block = BlockHelper.flylightStone;
									b0 = 0;
								}
							}
							...
							else if (l1 < 56 - l)
							{
								block = null;
								block1 = BlockHelper.flylightStone;
								p_150560_3_[i2] = BlockHelper.flylightStone;
							}
							else
							{
						...
						else if (k > 0)
						{
							...
							if (k == 0 && block1 == BlockHelper.flylightStone)
							{
	...
	public void populate(IChunkProvider par1IChunkProvider, int par2, int par3) {
		...
		for (i = 0; i < 5; i++) {
			...
			(new WorldGenFlylightTree(true)).generate(this.worldObj, this.rand, x, y, z);
		}	
		...
		if (TerrainGen.populate(par1IChunkProvider, worldObj, rand, var4, var5, false, LAKE) && rand.nextInt(4) == 0) {
			...
			(new WorldGenLakes(BlockHelper.water)).generate(this.worldObj, this.rand, var12, var13, var14);
		}
		...
		for(i = 0; i < 10; i++){
			...
			(ores.get(rand.nextInt(2))).generate(worldObj, rand, x, y, z);
		}
		...
		for(i = 0; i < 10; i++){
			...
			y = rand.nextInt(30);
			z = var5 + rand.nextInt(16) + 8;
			(lowerOres.get(rand.nextInt(5))).generate(worldObj, rand, x, y, z);
		}
		...
		for(i = 0; i < 10; i++){
			...
			y = rand.nextInt(200);
			z = var5 + rand.nextInt(16) + 8;
			(flowers.get(rand.nextInt(4))).generate(worldObj, rand, x, y, z);
		}
		...
		for(i = 0; i < 10; i++){
			...
			y = rand.nextInt(200);
			z = var5 + rand.nextInt(16) + 8;
			(sugarCane.get(rand.nextInt(1))).generate(worldObj, rand, x, y, z);
		}
	}
